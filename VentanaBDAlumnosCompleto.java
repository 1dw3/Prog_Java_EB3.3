...
public class VentanaBDAlumnosCompleto extends JFrame implements ListSelectionListener,RowSetListener,ActionListener,FocusListener{
	...
	// CachedRowSet
	private CachedRowSet crsAlumnos = null;
	private CachedRowSet crsAsignaturas = null;
	private CachedRowSet crsCalificaciones = null;
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					VentanaBDAlumnosCompleto frame = new VentanaBDAlumnosCompleto();
	...
	// JFrame
	public VentanaBDAlumnosCompleto() {
      ...
      crsAlumnos = myRowSetFactory.createCachedRowSet();
      crsAsignaturas = myRowSetFactory.createCachedRowSet();
      crsCalificaciones = myRowSetFactory.createCachedRowSet();
      
	    // selecciono todas las calificaciones
      crsCalificaciones.setCommand("SELECT * FROM calificaciones");
      crsCalificaciones.execute(conexion);
      
      // controlo los eventos
      crsCalificaciones.addRowSetListener(this);
					
			// cabeceras de las columnas
			ResultSetMetaData metaDatos = crsCalificaciones.getMetaData();
			...
			while (crsCalificaciones.next()) {
				Vector<String> fila = new Vector<String>();
				fila.add(crsCalificaciones.getString("dni"));
				fila.add(crsCalificaciones.getString("codasignatura"));
				fila.add(crsCalificaciones.getString("nota"));
				fila.add("\n\n\n\n\n\n\n");
        datosTabla.add(fila);
			}
	...
	public void cursorMoved(RowSetEvent rse) {
		...
		try {
			...
			fila = this.crsCalificaciones.getRow();
			// actualizo el valor
	...
	public void actionPerformed(ActionEvent ae) {
	    ...
	    try {
		    ...
		    //crs.setCommand("INSERT INTO alumnos VALUES ('00000000A','Nuevo','Alumno','1AS3')");
				crsAlumnos.setCommand(comando);
				crsAlumnos.execute(conexion);
			  conexion.close();
